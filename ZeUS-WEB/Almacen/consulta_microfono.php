...
if (!isset($_SESSION['login'])) {
	...
	if (isset($_SESSION["microfono"])) {
		$microfono = $_SESSION["microfono"];
		unset($_SESSION["microfono"]);
	}
...
<body>
	...
	<button id="myBtn" class="mybtn">Añadir micrófono </button>
		...
		<div class="modal-content">
			...
			<div class="modal-body">
				<form method="POST" action="almacen/controlador_microfono.php">
					...
					<label>Tipo de sujección: </label><div><textarea required type="text" id="sujemic" name="sujemic" rows="1" cols="40" maxlength="10"></textarea></div>
		...
		<table id="tabla1" style="width:100%">
			...
			foreach ($filas as $fila) {
				...
				<form method="POST" action="almacen/controlador_microfono.php">
					...
					<input id="ALIMENTACION" name="ALIMENTACION" type="hidden" value="<?php echo $fila["ALIMENTACION"]; ?>" />
					<input id="TIPOSUJECCION" name="TIPOSUJECCION" type="hidden" value="<?php echo $fila["TIPOSUJECCION"]; ?>" />
			

					<?php
					if (isset($microfono) and ($fila["REFERENCIA"] == $microfono["REFERENCIA"])) { ?>
						<!-- Editando título -->
						<tr>
						...
						<tr>
							...
							<td data-title="Alimentación:"><?php echo $fila['ALIMENTACION']; ?></td>
							<td data-title="Tipo de sujección:"><?php echo $fila['TIPOSUJECCION']; ?></td>
						<?php } ?>

						<?php if (isset($microfono) and $fila["REFERENCIA"] == $microfono["REFERENCIA"]) { ?>
